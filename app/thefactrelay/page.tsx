...
export default function TheFactRelayPage() {
    ...
    useEffect(() => {
        ...
        if (!isAdminUser && accountIdCookie) {
            ...
            const isValidAccount = TFR_ACCOUNTS.some(acc => acc.id === numericAccountId);
            if (isValidAccount) {
                setSelectedAccount(numericAccountId);
            } else {
                setError(`Invalid account ID: ${numericAccountId} is not a TheFactRelay account`);
            }
        }
    ...
    const fetchData = async (forceLive = false, accountOverride?: string) => {
        ...
        try {
            const requestBody: any = {
                ...
                adsenseAccountId: TFR_PUBLISHER_ID,
                adsenseAccountType: 'thefactrelay',
                forceLive,
            };

            if (account === 'all') {
                requestBody.accountIds = TFR_ACCOUNTS.map(a => a.id);
            } else {
            ...
            setError(err.message || 'Failed to fetch TheFactRelay data');
            setIsRefreshing(false);
        } finally {
            setLoading(false);
                        ...
                        <Col xs={24} md={8}>
                            <div>
                                ...
                                <Select
                                    ...
                                    disabled={!isAdmin}
                                >
                                    <Option value="all">All TheFactRelay Accounts</Option>
                                    {TFR_ACCOUNTS.map(acc => (
                            ...
                            {selectedAccount === 'all' && data.account_level_aggregated?.length > 0 && (
                                <Col span={24}>
                                    <Card title={<Title level={4}>Account-Level Performance</Title>}>
                                        <Table
                                            columns={[
                                                { title: 'Account', dataIndex: 'account_id', key: 'account_id', render: (id: string) => <Text strong>{TFR_ACCOUNTS.find(a => a.id === id)?.name || id}</Text> },
                            ...
                            <Col span={24}>
                                <div style={{ background: '#fff', padding: '20px', borderRadius: '8px' }}>
                                    ...
                                    {filteredCampaigns.length === 0 ? (
                                        <Alert message="No Data" description="No campaigns found for TheFactRelay." type="info" showIcon />
                                    ) : (
                                        <div style={{ overflowX: 'auto' }}>
                                            <style dangerouslySetInnerHTML={{
                                                __html: `
                        .tfr-table { width: 100%; border-collapse: collapse; font-size: 14px; }
                        .tfr-table thead tr:first-child th { padding: 12px; font-weight: 800; font-size: 14px; border: 3px solid #fff; }
                        .tfr-table thead tr:nth-child(2) th { padding: 8px; font-weight: 600; font-size: 12px; background: #fafafa; border: 1px solid #f0f0f0; }
                        .tfr-table tbody tr { transition: all 0.3s; }
                        .tfr-table tbody tr:hover { background: #f0f7ff !important; transform: translateY(-3px); box-shadow: 0 6px 20px rgba(24,144,255,0.1); }
                        .tfr-table tbody td { padding: 12px; border-bottom: 1px solid #f0f0f0; }
                        .tfr-name-cell { max-width: 220px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-weight: 500; color: #1f2937; }
                        `
                                            }} />
